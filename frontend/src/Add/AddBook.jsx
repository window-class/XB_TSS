...
function AddBook(){
 ...
 const submitForm = async(e)=>{
    e.preventDefault();
    try {
     const res = await axios.post("http://localhost:7000/api/addBook", book);
     console.log("Successfully added:", res);
     setMsg("Book recorded successfully, redirect to view page...");
     setMsgType("well");
     setTimeout(()=>{navigate('/GetBook');}, 3000);   
    } catch (error) {
     console.log("Error adding book:", error);
     setMsg(`Failed to record book: ${error}`);
     setMsgType("not");
    ...
    return(
        <div className="flex min-h-screen items-center justify-center bg-blue-400">
         <div className="w-2xl p-4 bg-gray-50 rounded-lg shadow-lg">
          <h1 className="text-2xl font-bold p-2 text-black">Record Books</h1>
          <form onSubmit={submitForm}>
           <div>
            ...
            <select name="SupplierId" autoComplete="off" required
                    className="w-full p-2 outline-1 focus:outline-none focus:ring-2 focus:ring-blue-400 rounded"
                    onChange={inputHandler}
                    >
            ...
            <button type="submit" className="mt-2 p-2 bg-green-500 text-white font-medium hover:bg-blue-600 rounded">
                record book</button>
            &nbsp;&nbsp;
            <Link to="/GetBook" className="text-blue-500 hover:underline">View book's List</Link>    
          </form>
         </div>
        </div>
    )
